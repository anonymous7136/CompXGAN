...
from compxgan.discriminators import (
    MultiPeriodDiscriminator,
    MultiResolutionDiscriminator,
)
from compxgan.feature_extractors import FeatureExtractor
from compxgan.heads import FourierHead
from compxgan.helpers import plot_spectrogram_to_numpy
from compxgan.loss import (
    DiscriminatorLoss,
    GeneratorLoss,
    FeatureMatchingLoss,
    MelSpecReconstructionLoss,
)
from compxgan.models import Backbone
from compxgan.modules import safe_log


class CompXGANExp(pl.LightningModule):
    def __init__(
        self,
    ...
    def on_validation_epoch_start(self):
        if self.hparams.evaluate_utmos:
            ...
            self.utmos_model = self.utmos_model.to(device=self.device)
            self.utmos_model = self.utmos_model.eval()
    ...
    def validation_step(self, batch, batch_idx, **kwargs):
        ...
        if self.hparams.evaluate_utmos:
            utmos_score = self.utmos_model(audio_hat_16khz, sr=16000).mean()
            utmos_score_gt = self.utmos_model(audio_16_khz, sr=16000).mean()
        else:
            utmos_score = torch.zeros(1, device=self.device)
            utmos_score_gt = torch.zeros(1, device=self.device)
    ...
    def validation_epoch_end(self, outputs):
        ...
        utmos_score_gt = torch.stack([x["utmos_score_gt"] for x in outputs]).mean()
        pesq_score = torch.stack([x["pesq_score"] for x in outputs]).mean()
        periodicity_loss = np.array(
            [x["periodicity_loss"].cpu().numpy() for x in outputs]
        ).mean()
        ...
        self.log("val/utmos_loss", utmos_score_gt - utmos_score, sync_dist=True)
