...
class cMultiResolutionDiscriminator(nn.Module):
    def __init__(
        self,
        fft_sizes: Tuple[int, ...] = (2048, 1024, 512),
        num_embeddings: Optional[int] = None,
        ...
        self.discriminators = nn.ModuleList(
            [
                DiscriminatorC(window_length=w, num_embeddings=num_embeddings)
                for w in fft_sizes
            ]
        )
...
class DiscriminatorC(nn.Module):
    def __init__(
        ...
        convs = lambda: nn.ModuleList(
            [
                cConv2d(1, channels, (3, 9), (1, 1), padding=(1, 4)),
                cConv2d(channels, channels, (3, 9), (1, 2), padding=(1, 4)),
                cConv2d(channels, channels, (3, 9), (1, 2), padding=(1, 4)),
                cConv2d(channels, channels, (3, 9), (1, 2), padding=(1, 4)),
                cConv2d(channels, channels, (3, 3), (1, 1), padding=(1, 1)),
            ]
        )
        self.band_convs = nn.ModuleList([convs() for _ in range(len(self.bands))])
        ...
        self.conv_post = cConv2d(channels, 1, (3, 3), (1, 1), padding=(1, 1))
